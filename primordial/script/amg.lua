--[[
    ...
    UI: "Premium Dark Easter" (Clean, Readable, Modern)
    Logic: Remote-Based Stats (Fixes CPH/Reset), Aggressive ESP
    Theme: Easter Event 2026 ✨
]]
...
local Colors = {
    Background = Color3.fromRGB(26, 26, 34),       -- Dark Purple-Grey (Sleek)
    PanelBg = Color3.fromRGB(35, 35, 45),          -- Lighter Grey for buttons
    TextWhite = Color3.fromRGB(255, 255, 255),     -- Pure White (Readable)
    SubText = Color3.fromRGB(180, 180, 190),       -- Light Grey
    AccentPurple = Color3.fromRGB(180, 150, 255),  -- Premium Pastel Purple
    AccentGreen = Color3.fromRGB(110, 230, 150),   -- Soft Mint Green (For toggles)
    EggPink = Color3.fromRGB(255, 170, 200),       -- Pastel Pink
    EggBlue = Color3.fromRGB(150, 210, 255),       -- Pastel Blue
    DangerRed = Color3.fromRGB(255, 90, 90)        -- Red for Murderer
}

-- 3. GAME REMOTES (LOGIC UNTOUCHED)
...
ScreenGui.Name = "MM2EasterPremium"
...
-- 🥚 SUBTLE FALLING EGGS (Behind everything, 80% transparent)
local EggColors = {Colors.EggPink, Colors.AccentPurple, Colors.EggBlue, Colors.AccentGreen}
task.spawn(function()
    while true do
        if MainFrame.Visible then
            local egg = Instance.new("Frame")
            egg.Parent = MainFrame
            egg.ZIndex = 0
            egg.BackgroundColor3 = EggColors[math.random(1, #EggColors)]
            egg.BorderSizePixel = 0
            
            local size = math.random(5, 9)
            egg.Size = UDim2.new(0, size, 0, size * 1.3)
            egg.Position = UDim2.new(math.random(), 0, -0.1, 0)
            egg.BackgroundTransparency = 0.85 -- Very subtle!
            
            egg.Rotation = math.random(-20, 20)
            local fCorner = Instance.new("UICorner"); fCorner.CornerRadius = UDim.new(0.5, 0); fCorner.Parent = egg
            
            local tweenInfo = TweenInfo.new(math.random(4, 8), Enum.EasingStyle.Linear)
            local endPos = UDim2.new(egg.Position.X.Scale + (math.random(-1,1)/10), 0, 1.1, 0)
            local tween = TweenService:Create(egg, tweenInfo, {Position = endPos, Rotation = egg.Rotation + math.random(-30, 30)})
            tween:Play()
            Debris:AddItem(egg, 8)
        end
        wait(0.3)
    end
end)
...
SubTitle.Text = "✨ EASTER EVENT 2026 ✨"
SubTitle.TextColor3 = Colors.AccentPurple
...
-- Tabs
local TabContainer = Instance.new("ScrollingFrame"); TabContainer.Parent = MainFrame; TabContainer.BackgroundTransparency = 1; TabContainer.Position = UDim2.new(0, 10, 0, 52); TabContainer.Size = UDim2.new(1, -20, 0, 35); TabContainer.CanvasSize = UDim2.new(1.5, 0, 0, 0); TabContainer.ScrollBarThickness = 0
...
local function CreateTabBtn(text)
    local Btn = Instance.new("TextButton"); Btn.Parent = TabContainer; Btn.BackgroundColor3 = Colors.PanelBg; Btn.BorderSizePixel = 0; Btn.Size = UDim2.new(0, 80, 1, 0); Btn.Font = Enum.Font.GothamBold; Btn.Text = text; Btn.TextColor3 = Colors.SubText; Btn.TextSize = 12
    ...
    Btn.MouseButton1Click:Connect(function() 
        for _, c in pairs(TabContainer:GetChildren()) do 
            if c:IsA("TextButton") then c.BackgroundColor3 = Colors.PanelBg; c.TextColor3 = Colors.SubText; c:FindFirstChild("UIStroke").Transparency = 1 end 
        end
...
local function CreateButton(tabName, text, icon, callback)
    local Frame = Instance.new("Frame"); Frame.Parent = ContentContainer; Frame.BackgroundColor3 = Colors.PanelBg; Frame.Size = UDim2.new(1, 0, 0, 40); Frame.Visible = (tabName == "EASTER"); local Corner = Instance.new("UICorner"); Corner.CornerRadius = UDim.new(0, 6); Corner.Parent = Frame
    ...
    local TextLbl = Instance.new("TextLabel"); TextLbl.Parent = Frame; TextLbl.BackgroundTransparency = 1; TextLbl.Position = UDim2.new(0, 45, 0, 0); TextLbl.Size = UDim2.new(0.7, 0, 1, 0); TextLbl.Font = Enum.Font.GothamMedium; TextLbl.Text = text; TextLbl.TextColor3 = Colors.TextWhite; TextLbl.TextSize = 13; TextLbl.TextXAlignment = Enum.TextXAlignment.Left
    local Btn = Instance.new("TextButton"); Btn.Parent = Frame; Btn.BackgroundTransparency = 1; Btn.Size = UDim2.new(1, 0, 1, 0); Btn.Text = ""
    Btn.MouseButton1Click:Connect(function() 
        TweenService:Create(Frame, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(55, 55, 70)}):Play(); wait(0.1); 
        TweenService:Create(Frame, TweenInfo.new(0.1), {BackgroundColor3 = Colors.PanelBg}):Play(); 
        callback() 
    end)
...
local function CreateToggle(tabName, text, icon, callback)
    ...
    local Status = Instance.new("Frame"); Status.Parent = Frame; Status.BackgroundColor3 = Color3.fromRGB(60, 60, 75); Status.Position = UDim2.new(0.88, 0, 0.5, -6); Status.Size = UDim2.new(0, 12, 0, 12); local SCorner = Instance.new("UICorner"); SCorner.CornerRadius = UDim.new(1,0); SCorner.Parent = Status
    local enabled = false
    Btn.MouseButton1Click:Connect(function() 
        enabled = not enabled
        if enabled then 
            TweenService:Create(Status, TweenInfo.new(0.3), {BackgroundColor3 = Colors.AccentGreen}):Play()
            TweenService:Create(Frame:FindFirstChild("TextLabel", true), TweenInfo.new(0.3), {TextColor3 = Colors.AccentGreen}):Play()
        else 
            TweenService:Create(Status, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(60, 60, 75)}):Play()
            TweenService:Create(Frame:FindFirstChild("TextLabel", true), TweenInfo.new(0.3), {TextColor3 = Colors.TextWhite}):Play()
        end
        callback(enabled) 
    end)
    return Frame
...
local function CreateInput(tabName, placeholder, callback)
    local Frame = Instance.new("Frame"); Frame.Parent = ContentContainer; Frame.BackgroundColor3 = Colors.PanelBg; Frame.Size = UDim2.new(1, 0, 0, 40); Frame.Visible = (tabName == "EASTER"); local Corner = Instance.new("UICorner"); Corner.CornerRadius = UDim.new(0, 6); Corner.Parent = Frame
    local Box = Instance.new("TextBox"); Box.Parent = Frame; Box.BackgroundTransparency = 1; Box.Size = UDim2.new(1, -20, 1, 0); Box.Position = UDim2.new(0, 10, 0, 0); Box.Font = Enum.Font.Gotham; Box.PlaceholderText = placeholder; Box.Text = ""; Box.TextColor3 = Colors.TextWhite; Box.PlaceholderColor3 = Colors.SubText; Box.TextSize = 13
    Box.FocusLost:Connect(function() callback(Box.Text) end)
    table.insert(Elements, {Tab = tabName, Frame = Frame})
end

local function CreateSlider(tabName, text, min, max, default, callback)
    local Frame = Instance.new("Frame"); Frame.Parent = ContentContainer; Frame.BackgroundColor3 = Colors.PanelBg; Frame.Size = UDim2.new(1, 0, 0, 50); Frame.Visible = (tabName == "EASTER"); local Corner = Instance.new("UICorner"); Corner.CornerRadius = UDim.new(0, 6); Corner.Parent = Frame
    ...
    local ValLbl = Instance.new("TextLabel"); ValLbl.Parent = Frame; ValLbl.BackgroundTransparency = 1; ValLbl.Position = UDim2.new(0.8, 0, 0, 5); ValLbl.Size = UDim2.new(0.15, 0, 0, 20); ValLbl.Font = Enum.Font.GothamBold; ValLbl.Text = tostring(default); ValLbl.TextColor3 = Colors.AccentPurple; ValLbl.TextSize = 13
    local SliderBar = Instance.new("Frame"); SliderBar.Parent = Frame; SliderBar.BackgroundColor3 = Color3.fromRGB(60, 60, 75); SliderBar.Position = UDim2.new(0.05, 0, 0.7, 0); SliderBar.Size = UDim2.new(0.9, 0, 0, 6); local BarCorner = Instance.new("UICorner"); BarCorner.CornerRadius = UDim.new(1,0); BarCorner.Parent = SliderBar
...
local function CreateKeybind(tabName, text, defaultKey, callback)
    ...
    local BindLbl = Instance.new("TextLabel"); BindLbl.Parent = Frame; BindLbl.BackgroundTransparency = 1; BindLbl.Position = UDim2.new(0.7, 0, 0, 0); BindLbl.Size = UDim2.new(0.25, 0, 1, 0); BindLbl.Font = Enum.Font.GothamBold; BindLbl.Text = "[" .. defaultKey.Name .. "]"; BindLbl.TextColor3 = Colors.SubText; BindLbl.TextSize = 12
...
local function getCoins()
    ...
    if #targets == 0 then 
        for _, v in pairs(Workspace:GetDescendants()) do 
            if (v.Name == "Candy" or v.Name == "Egg" or v.Name == "EasterEgg") and v:IsA("BasePart") then table.insert(targets, v) end 
        end 
    end
    return targets
end

-- == TABS POPULATION (Easter Premium) ==
...
CreateToggle("PVP", "KILL ALL (Murderer)", "🔪", function(state)
    Settings.KillAll = state
    if state then
        task.spawn(function()
            while Settings.KillAll do
                local hasKnife = LocalPlayer.Character:FindFirstChild("Knife")
                if hasKnife then
...
CreateToggle("PVP", "AUTO GRAB GUN", "🔫", function(state)
    ...
    if Settings.EspEnabled then
        for _, plr in pairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character then
                local hl = plr.Character:FindFirstChild("EasterESP")
                if not hl then hl = Instance.new("Highlight", plr.Character); hl.Name = "EasterESP"; hl.FillTransparency = 0.5 end
                local col = Colors.EggBlue -- Innocent
                if plr.Backpack:FindFirstChild("Knife") or plr.Character:FindFirstChild("Knife") then col = Colors.DangerRed -- Murderer
                elseif plr.Backpack:FindFirstChild("Gun") or plr.Character:FindFirstChild("Gun") then col = Colors.AccentGreen end -- Sheriff
...
-- Stats Footer (Clean & Sleek)
local StatsPanel = Instance.new("Frame"); StatsPanel.Parent = MainFrame; StatsPanel.BackgroundColor3 = Color3.fromRGB(15, 15, 20); StatsPanel.BorderSizePixel = 0; StatsPanel.Position = UDim2.new(0, 10, 1, -70); StatsPanel.Size = UDim2.new(1, -20, 0, 60); 
local SP_Corner = Instance.new("UICorner"); SP_Corner.CornerRadius = UDim.new(0, 8); SP_Corner.Parent = StatsPanel
local CPH_Display = Instance.new("TextLabel"); CPH_Display.Parent = StatsPanel; CPH_Display.BackgroundTransparency = 1; CPH_Display.Position = UDim2.new(0.05, 0, 0.1, 0); CPH_Display.Size = UDim2.new(0.9, 0, 0.4, 0); CPH_Display.Font = Enum.Font.GothamBold; CPH_Display.TextColor3 = Colors.AccentPurple; CPH_Display.TextSize = 12; CPH_Display.TextXAlignment = Enum.TextXAlignment.Left; CPH_Display.Text = "💰 CPH: 0"
local Coll_Display = Instance.new("TextLabel"); Coll_Display.Parent = StatsPanel; Coll_Display.BackgroundTransparency = 1; Coll_Display.Position = UDim2.new(0.05, 0, 0.5, 0); Coll_Display.Size = UDim2.new(0.9, 0, 0.4, 0); Coll_Display.Font = Enum.Font.GothamMedium; Coll_Display.TextColor3 = Colors.TextWhite; Coll_Display.TextSize = 12; Coll_Display.TextXAlignment = Enum.TextXAlignment.Left; Coll_Display.Text = "🧺 Collected: 0"
...
-- Minimize Button (Premium Look)
local MinBtn = Instance.new("ImageButton"); MinBtn.Parent = ScreenGui; MinBtn.Name = "OpenGift"; MinBtn.BackgroundColor3 = Colors.Background; MinBtn.Position = UDim2.new(0, 20, 0.9, -20); MinBtn.Size = UDim2.new(0, 50, 0, 50)
...
local OpenLbl = Instance.new("TextLabel"); OpenLbl.Parent = MinBtn; OpenLbl.BackgroundTransparency = 1; OpenLbl.Size = UDim2.new(1,0,1,0); OpenLbl.Text = "🐰"; OpenLbl.TextSize = 25
local visible = true
MinBtn.MouseButton1Click:Connect(function() visible = not visible; MainFrame.Visible = visible end)

print("🐰 Premium Dark Easter Hub Loaded! 🌸")
